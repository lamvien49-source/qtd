...
- **Phía quản trị** (`/#/admin`): quản lý khách hàng (nhập **1 ô địa chỉ**, hệ thống tự tách Xóm/Thôn/Tỉnh) — **đọc trực tiếp file Excel sổ theo dõi vay đang dùng (`.xls` hoặc `.xlsx`)** tải lên, tự khớp đúng cột, tự cập nhật **hồ sơ** khách hàng (không tự tạo tài khoản đăng nhập — xem mục "Hồ sơ khách hàng vs. tài khoản đăng nhập" bên dưới), không cần thư viện ngoài. Trang **Khách hàng & Hợp đồng**: tên khách hàng luôn hiện trọn 1 dòng riêng (không bị cắt bớt dù tên dài); mỗi khách chỉ 1 hợp đồng thì hiện gọn **Gốc (= dư nợ hiện tại) / Lãi** ngang hàng với thông tin khách, bấm vào là ra thẳng chi tiết hợp đồng; khách nhiều hợp đồng thì mỗi hợp đồng 1 dòng riêng (mã hợp đồng, trạng thái, Gốc/Lãi) — **bấm vào mới ra đầy đủ chi tiết** (số tiền vay ban đầu, dư nợ, ngày vay, ngày đến hạn, đã trả lãi đến ngày, lãi suất, lãi đến nay, SĐT) giống hệt màn hình khách hàng, kèm nút **nhắn SMS báo lãi cho khách** ngay tại đó — **dữ liệu hợp đồng chỉ đọc, không có ô sửa** (lấy từ Excel, muốn cập nhật thì nhập lại file mới). Có thể **lọc Thôn/Xóm chọn nhiều mục cùng lúc** và **sắp xếp theo Gốc/Lãi tăng dần-giảm dần**. Số điện thoại bấm vào **gọi luôn** (`tel:`). Trang **Tổng quan** có thêm ô đếm + danh sách **hợp đồng gần đến hạn**, tiêu đề tô màu kèm số lượng để dễ chú ý. Ngoài ra: xem/xử lý yêu cầu tư vấn, chỉnh banner + thông tin quỹ tín dụng + thông tin nhận thanh toán (QR).
- **Quản lý User**: 1 trang duy nhất quản lý **toàn bộ tài khoản trong hệ thống** — cả "Use" (khách hàng) lẫn "Quản trị viên" (toàn quyền hoặc chỉ xem) — chỉ quản trị viên toàn quyền vào được. Hiện số lượng Use/Quản trị viên đã tạo, lọc theo loại tài khoản, bấm vào 1 tài khoản để xem thông tin + cấp lại mật khẩu bất cứ lúc nào. Nút **"Tạo User"** có mục chọn loại tài khoản: **Use** (chỉ cần CCCD + họ tên + SĐT, **không còn ô địa chỉ** — địa chỉ tự lấy từ hồ sơ Excel nếu CCCD đó đã có sẵn + **mật khẩu đăng nhập** tự đặt hoặc để trống cho tự sinh ngẫu nhiên, luôn hiện ra sau khi lưu để gửi cho khách; Use chỉ xem được hợp đồng của mình, đăng nhập được và tự động dò thấy mọi hợp đồng khớp CCCD ngay sau khi tạo) hoặc **Quản trị viên** (tên đăng nhập + mật khẩu tự đặt hoặc tự sinh + chọn vai trò Toàn quyền/Chỉ xem — chọn "Chỉ xem" sẽ hiện thêm mục **phân quyền theo Thôn/Xóm** ngay trong form, các Xóm hiển thị dạng nút bấm gọn đẹp). Mọi loại User (khách hàng lẫn quản trị viên) đều có mục **"Đổi mật khẩu"** tự chọn bất cứ lúc nào (sidebar + bảng "Thêm" trên di động), không chỉ riêng lần đăng nhập đầu bắt buộc.
...
- **Tải file lên = danh sách hợp đồng đầy đủ hiện tại (full-sync)**: hợp đồng nào đang có trong hệ thống mà **không còn xuất hiện** trong file vừa tải sẽ **tự động bị xóa** để hệ thống luôn khớp đúng với file mới nhất — dùng khi tải lại toàn bộ sổ theo dõi vay mới nhất. Ngược lại, mục **"dán dữ liệu thủ công"** (copy từ Excel, dùng cho vài dòng lẻ) **không** áp dụng full-sync — chỉ thêm/cập nhật, không xóa hợp đồng nào khác, an toàn khi chỉ muốn bổ sung nhanh vài dòng.
- **Nhập từ Excel/dán tay chỉ cập nhật hồ sơ (tên, SĐT, địa chỉ) + hợp đồng — KHÔNG tự tạo tài khoản đăng nhập.** Khách hàng mới xuất hiện trong file chỉ có "hồ sơ" (đủ để gắn hợp đồng, admin xem được trong Khách hàng & Hợp đồng) nhưng **chưa đăng nhập được** cho tới khi admin bấm **"Tạo User"** cấp tài khoản riêng cho họ (xem mục kế tiếp).

## Hồ sơ khách hàng vs. tài khoản đăng nhập (quan trọng)

Bản demo tách riêng 2 khái niệm:
- **Hồ sơ khách hàng** (tên, CCCD, SĐT, địa chỉ, hợp đồng) — do **Excel quyết định**, luôn khớp đúng file mới nhất tải lên (full-sync, xem mục trên). Hồ sơ mới có thể chưa có ai đăng nhập được.
- **Tài khoản đăng nhập** (CCCD/SĐT + mật khẩu) — do **admin tự tạo tay** ở nút "Tạo User", **hoàn toàn tách rời** khỏi việc nhập Excel. Tạo xong là đăng nhập được ngay, không cần chờ hay làm thêm bước nào.

Khi admin bấm "Tạo User" với 1 số CCCD **đã có sẵn hồ sơ** (từ Excel), hệ thống chỉ gắn thêm tài khoản vào đúng hồ sơ đó — khách đăng nhập là **tự thấy ngay mọi hợp đồng khớp CCCD**, không cần nhập lại địa chỉ hay bất kỳ thông tin nào khác. Việc dò hợp đồng theo CCCD diễn ra **mỗi lần hiển thị** (không lưu cứng liên kết theo thời điểm tạo), nên:
...
- Hồ sơ nhập từ Excel mà **chưa được "Tạo User"** thì khi thử đăng nhập sẽ báo rõ: "CCCD/SĐT này chưa được cấp tài khoản đăng nhập — liên hệ quỹ tín dụng để được tạo tài khoản."
...
- Chọn **Use** → chỉ cần CCCD + họ tên + SĐT (**không còn ô địa chỉ** — nếu CCCD đã có hồ sơ từ Excel thì địa chỉ tự lấy từ đó; nên nhập cả CCCD lẫn SĐT vì Use đăng nhập được bằng cả 2 số) — không có mục phân quyền vì Use chỉ xem được hợp đồng của mình. Tạo xong đăng nhập được ngay và tự động thấy mọi hợp đồng khớp CCCD.
