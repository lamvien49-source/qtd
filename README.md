...
- Nút chính **"Gửi ảnh QR sang app ngân hàng"** (nút màu nổi bật, đứng đầu tiên): dùng Web Share API chuẩn của trình duyệt (`navigator.share`) để mở bảng **"Mở bằng"** có sẵn của điện thoại (giống bảng chọn app khi bấm gọi điện, hoặc khi chia sẻ ảnh từ Zalo/Ảnh...), khách chọn đúng app ngân hàng/ví đang cài để gửi ảnh QR sang đó. **Đặt tên đúng bản chất, không gọi là "thanh toán"/"mở app thanh toán"** vì dễ hiểu nhầm là tự mở thẳng màn chuyển khoản trong app — **thực tế đây chỉ là chia sẻ ảnh**: hầu hết app ngân hàng KHÔNG tự mở đúng màn hình chuyển khoản kèm sẵn số tiền/nội dung chỉ từ việc nhận ảnh chia sẻ (khác với việc quét QR trực tiếp bằng camera trong app); khách vẫn cần tự vào app đó bấm "Quét QR từ ảnh" (nếu app có tính năng này) hoặc quét trực tiếp bằng camera như mô tả bên dưới. Đã thử cách deep-link thẳng vào 1 app cụ thể qua `dl.vietqr.io` nhưng **xác nhận không hoạt động** (dịch vụ này bắt buộc tham số "app" theo đúng mã ngân hàng, không có cách nào xác minh được mã đúng trong môi trường phát triển) nên không dùng cách đó — trong 1 bản demo không kết nối API ngân hàng thật, chưa có cách nào tự mở thẳng đúng màn chuyển khoản trong app bất kỳ.
