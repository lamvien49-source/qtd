...
Deno.serve(async (req) => {
  ...
  if (role === 'customer') {
    ...
    if (error) {
      console.error('query customers error:', error); // chi tiết xem ở Edge Functions → Logs, không trả về client
    ...
    if (error) {
      console.error('query admins error:', error); // chi tiết xem ở Edge Functions → Logs, không trả về client
      return json({ ok: false, reason: 'Lỗi hệ thống, thử lại sau.' }, 500);
    }
    row = data;
  }
